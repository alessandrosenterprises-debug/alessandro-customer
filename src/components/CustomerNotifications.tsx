...
export function CustomerNotifications() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`customer-notices:${user.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'customer_notifications', filter: `customer_id=eq.${user.id}` }, () => void load())
      .subscribe();
  ...
  return <div className="customer-notification">
    <button className="customer-bell" aria-label="Customer notifications" onClick={() => setOpen((value) => !value)}>
      {'\u1F514'}
      {unread > 0 && <i>{unread}</i>}
    </button>
