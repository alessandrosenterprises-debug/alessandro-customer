...
export function CustomerNotifications() {
  ...
  async function load() {
    if (!user) return;
    const { data } = await supabase.from('customer_notifications').select('*').eq('customer_id', user.id).order('created_at', { ascending: false }).limit(20);
    setItems((data ?? []) as Notice[]);
  }
  useEffect(() => {
    void load();
    if (!user) return;
    const channel = supabase.channel(`customer-notices:${user.id}`).on('postgres_changes', { event: '*', schema: 'public', table: 'customer_notifications', filter: `customer_id=eq.${user.id}` }, () => void load()).subscribe();
    return () => { void supabase.removeChannel(channel); };
  }, [user]);
  const unread = items.filter((item) => !item.read_at).length;
  async function markRead(item: Notice) {
    if (!user || item.read_at) return;
